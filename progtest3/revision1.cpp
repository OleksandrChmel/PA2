...
CDate CDate::operator+(const Date * p_Date) {
    ...
    if(p_Date->g_Year && !p_Date->g_Month){
        ...
        if((g_Day+p_Date->g_Day+g_LeapYear)<DAYSINMONTH[g_Month] && (g_Day+p_Date->g_Day+g_LeapYear)>0)
            l_.setG_Day(g_Day+p_Date->g_Day);
        else if(p_Date->g_Day<0 && g_Day+DAYSINMONTH[g_Month-1]+p_Date->g_Month>0){
            l_.setG_Month(g_Month-1);
            l_.setG_Day(g_Day+DAYSINMONTH[g_Month-1]+p_Date->g_Day);
        }
        else{
            long int l_Total  = DateToDays() + p_Date->g_Day;
            uint64_t l_dy = (uint64_t)(l_Total-l_Total/365/4+l_Total/365/100-l_Total/365/400+l_Total/365/4000)/365;
            uint64_t l_ddm = (uint64_t)(l_Total-l_Total/365/4+l_Total/365/100-l_Total/365/400+l_Total/365/4000)%365;
            l_.setG_Year(l_dy);
            l_.setLeapYear(l_dy);
            if(l_ddm==0){
                l_.setG_Month(1);
            ...
            int l_dm = 0;
            while (l_ddm>DAYSINMONTHSUM[l_dm] && l_dm<13)
                l_dm++;
            if(l_.isLeapYear() && l_dm>2) {
                l_dm=0;
                while (l_ddm-1>DAYSINMONTHSUM[l_dm] && l_dm<13)
                    l_dm++;
            }
            l_.setG_Month(l_dm-1);
            l_.setG_Day(l_ddm-DAYSINMONTHSUM[l_dm-1]);
