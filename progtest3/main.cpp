...
ostream& operator << (ostream & p_ss,const CDate & p_Date) {

    ios_base::fmtflags f = p_ss.flags();
    p_ss.unsetf(f);

    char l_f = p_ss.fill();

    p_ss << setw(4) << setfill('0') << p_Date.getG_Year() << "-"
            << setw(2) <<setfill('0') << (int)p_Date.getG_Month() << "-"
            << setw(2) <<setfill('0') << (int)p_Date.getG_Day();

    return p_ss << setiosflags(f) << setfill(l_f);
}


//---------------------------------------------------------LITERALS-----------------------------------------------------

UDate operator "" _year(unsigned long long int p_Year){
    return Year(p_Year);
}

UDate operator "" _years(unsigned long long int p_Year){
    return Year(p_Year);
}

UDate operator "" _month(unsigned long long int p_Month){
    return Month(p_Month);
}

UDate operator "" _months(unsigned long long int p_Month){
    return Month(p_Month);
}

UDate operator "" _day(unsigned long long int p_Day){
    return Day(p_Day);
}

UDate operator "" _days(unsigned long long int p_Day){
    return Day(p_Day);
}
